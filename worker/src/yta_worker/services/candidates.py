...
def upsert_tracker_candidates(
    database_session: Session, tracker: Tracker, candidate_video_ids: list[str]
) -> None:
    ...
    if len(candidates_sorted_by_recency) > tracker.candidate_pool_size:
        for candidate_to_remove in candidates_sorted_by_recency[
            tracker.candidate_pool_size :
        ]:
