...
def run_tracker_discovery(
    database_session: Session, youtube_client: YouTubeClient, tracker: Tracker
) -> None:
    if tracker.type == TrackerType.channel:
        ...
        if not uploads_playlist_id:
            return
        candidate_video_ids = youtube_client.list_playlist_video_ids(
            uploads_playlist_id, tracker.candidate_pool_size
        )
    ...
    if tracker.type == TrackerType.search:
        if not tracker.search_query:
            return
        candidate_video_ids = youtube_client.search_video_ids(
            tracker.search_query, tracker.candidate_pool_size
        )
        upsert_tracker_candidates(database_session, tracker, candidate_video_ids)
